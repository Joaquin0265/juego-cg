...
void Pantalla_MenuPrincipal::moverSeleccion(int direccion) {
    ...
    // Bucle para no salirnos de los límites del menú
...
void Pantalla_MenuPrincipal::actualizar() {
    // Animaciones del menú o efectos de neón en el futuro
}

void Pantalla_MenuPrincipal::renderizar() {
    // 1. Fondo Degradado Noche/Neón (Procedural)
    glBegin(GL_QUADS);
        // Arriba (Violeta Oscuro)
    ...
    // Opción 1: 1v1 Local
    if (opcionSeleccionada == 0) {
        dibujarTexto("> 1 vs 1 LOCAL <", -0.22f, 0.0f, GLUT_BITMAP_HELVETICA_18, 0.0f, 1.0f, 0.5f); // Verde
    } else {
        dibujarTexto("  1 vs 1 LOCAL  ", -0.22f, 0.0f, GLUT_BITMAP_HELVETICA_18, 0.7f, 0.7f, 0.7f); // Gris
    }

    // Opción 2: Salir
    if (opcionSeleccionada == 1) {
        dibujarTexto("> SALIR <", -0.12f, -0.18f, GLUT_BITMAP_HELVETICA_18, 1.0f, 0.2f, 0.2f); // Rojo
    } else {
        dibujarTexto("  SALIR  ", -0.12f, -0.18f, GLUT_BITMAP_HELVETICA_18, 0.7f, 0.7f, 0.7f); // Gris
    }

    // Instrukciones en la parte inferior
